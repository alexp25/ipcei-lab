...
status_t ili9341_init(ili9341_t *lcd, const ili9341_config_t *config)
{
    ...
    master_config.baudRate = lcd->config.spi_baudrate_hz;
    master_config.bitsPerFrame = 8U;
    master_config.cpol = kLPSPI_ClockPolarityActiveHigh;
    master_config.cpha = kLPSPI_ClockPhaseFirstEdge;
    master_config.whichPcs = kLPSPI_Pcs0;
    master_config.pcsActiveHighOrLow = kLPSPI_PcsActiveLow;
    master_config.pinCfg = kLPSPI_SdiInSdoOut;
    LPSPI_MasterInit(lcd->config.spi, &master_config, lcd->config.spi_source_clock_hz);
